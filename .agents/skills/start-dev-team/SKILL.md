...
3. 対象リポジトリの絶対パス、確認済みの要件、維持する操作経路、制約、対象ファイル、完了条件だけを渡す。仮説や検討案は現在の判断に必要な場合だけ分離して含める。テストを依頼する場合は期待する挙動と再発条件を渡す。
...
差分と必要な経路を依頼文へ渡した場合は、メモリのクイックパス、README・AGENTSの再読、差分外のファイル一覧取得、依頼されていないskill・テスト・lint・format・ビルドを実行しないことを停止条件として明記する。
結果を待つときは`wait_agent`の`timeout_ms: 60000`を基本とする。`timed_out`は指定期間内に結果が返らなかったことだけを示すため、agentが`running`なら同じ依頼を重複起動せず、追加で待機する。
