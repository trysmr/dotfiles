...
3. 確認済みの要件、維持する操作経路、制約、対象ファイル、完了条件をsubagentごとに分けて渡す。仮説や検討案は確認済みの要件と分離し、現在の判断に必要な場合だけ含める。テストを依頼する場合は期待する挙動と再発条件を渡し、確認されていないアサーション候補を作業項目にしない。
...
- subagentへの依頼と結果は、現在の会話を読めない新規参加者にも対象と背景が分かるよう、具体的な名前、操作、状態、パスを使う。
