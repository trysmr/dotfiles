...
description: Terraform構成が品質特性をどこまで支えるかを静的にreviewし、確認できる対策、確認できない実動作、検証方法、安全上の制約、証拠の不足を整理する。「Terraform構成の品質特性を確認」「terraform validate、test、condition、check、Policy as Codeの使い分け」「stateやplanを読まずにTerraformをreview」などの依頼で使用。
---

# Review Terraform Quality
...
2. リポジトリの指示を確認し、[Terraformによる対策と確認](references/terraform-controls.md)を読む。
...
`engineer-quality-attributes`から利用された場合は、次を返して品質シナリオへ統合できるようにする。
...
品質範囲reviewでは、9特性の判定表に加え、有益性、リスクからの保護、受容性の判定表を独立して必ず出力する。Terraformだけでは評価できなくても省略せず、各特性を`判断待ち`とする理由と必要な証拠を示す。そのうえで、証拠の対応、判断待ち、次の作業を出力する。確認手段の選択では、目的、選択候補、停止するか警告か、副作用、確認できない範囲を簡潔に出力する。
