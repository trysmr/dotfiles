...
- 各アサーションが守る、確認済みの挙動または再発条件を説明できるようにする。説明できないアサーションは追加しない。
- 否定アサーションは、不在そのものが明示された挙動、認可やセキュリティの境界、または具体的な不具合の再発条件である場合に使う。
- 要件で期待される表示や結果を直接検証し、会話に登場した語や検討案から新しい不在条件を作らない。
