...
sandbox内でGitHub認証を利用できず、`git fetch`または`gh`が失敗した場合は、認証情報の保護を緩めない。必要な確認済みコマンドをユーザーへ提示し、その出力を受け取ってから次の手順へ進む。対象PRの内容、作成中のrelease PRがないこと、作成結果の確認に必要な出力が得られない間は、PR作成コマンドを提示しない。

1. `git fetch origin`でremoteの情報を更新する。
...
タイトルと本文を1行ずつ読み、漢語接頭辞の造語、助詞の省略、既存の語彙にない業務用語、禁止語がないか照合する。あわせて、同じ語や内容を不要に反復していないか、概要がタイトルや変更点の言い換えだけでWhyを欠いていないか、「ため」「ので」で結ぶ原因と結果が確認済みかを確認する。指摘がなくても次の表を出力し、`gh pr create`の前にユーザーの許可を得る。
...
| 変更点1行目 | （原文） | ... | ... |
| テスト計画1行目 | （原文） | ... | ... |
| 全体 | （反復、Whyの不足、理由と行為のつながり） | OK / 問題と該当箇所 | （修正案。OKなら空欄） |

本文の行数に合わせて行を増減し、見出しを除く各行を省略せず照合する。

## PR作成コマンド

PR本文はWriteで一時ファイルへ書き、`--body-file`で渡す。本文をシェルのコードへ埋め込まず、`gh pr create`は単独で実行する。
